...
function BestWorst({ summary }: { summary: PortfolioSummary }) {
    const rows = [summary.best, summary.worst].filter(Boolean);
    if (!rows.length) return null;

    return (
        <div className="panel detail-panel">
            <span className="mono detail-panel__title">Best / worst position</span>
            {rows.map(p => (
                <Link key={`${p!.game}-${p!.id}`} className="bw-row" to={`/catalog/${p!.game}/${p!.id}`}>
                    <img className="bw-row__thumb" src={p!.pictureUrl} alt="" loading="lazy" />
                    <span className="bw-row__name">{p!.name}</span>
