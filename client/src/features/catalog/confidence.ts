...
const LEVELS: Record<string, { label: string; short: string; cls: string; reason: string }> = {
    high: {
        ...
        reason: "The model's own 80% scenario range for this forecast is tight — its optimistic and pessimistic cases mostly agree.",
    },
    med: {
        label: 'Medium confidence', short: 'MED', cls: 'conf--med',
        reason: "The model's 80% scenario range is moderately wide — treat the point forecast as a rough center, not a target.",
    },
    low: {
        label: 'Low confidence', short: 'LOW', cls: 'conf--low',
        reason: "The model's 80% scenario range is wide — it genuinely isn't sure, and the low–high band matters more than the point forecast.",
    },
};
