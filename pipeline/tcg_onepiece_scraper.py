#!/usr/bin/env python3
"""
TCGplayer One Piece Card Game scraper.

Collects EVERY One Piece card from TCGplayer's public storefront endpoints:
  - Full card info (name, set, rarity, number, color, cost, power, type, text, etc.)
...
Usage:
    ...
    python tcg_onepiece_scraper.py                 # full run
    python tcg_onepiece_scraper.py --limit 50      # quick test (first 50 cards)
    python tcg_onepiece_scraper.py --no-images      # skip image download
    python tcg_onepiece_scraper.py --delay 1.5      # seconds between requests

    # Recommended recurring/weekly run: refresh price history for existing cards
    # older than 7 days, and fully fetch new cards (history + missing images).
    python tcg_onepiece_scraper.py --refresh-days 7

Per card in Phase 2 the scraper fetches price history when it is new or stale
(--refresh-days) and downloads the image only when it is missing on disk, so a
recurring run backfills new price points for existing cards and everything for
new cards without re-downloading existing images.
...
PRODUCT_LINE = "one-piece-card-game"   # TCGplayer product-line url slug
...
DB_PATH = os.path.join(DATA_DIR, "onepiece_cards.db")
...
HEADERS = {
    ...
    "Referer": "https://www.tcgplayer.com/search/one-piece-card-game/product",
...
def is_single_card(product):
    ...
    (e.g. "ST05-002") and a real rarity ("Common", "Leader", ...). Sealed
...
def iter_all_products(session, base_delay, limit=None, singles_only=True):
    """Yield every product dict for the One Piece product line."""
    offset = 0
    total = None
    fetched = 0
    while True:
        ...
        if limit is not None:
            ...
            if size <= 0:
                return
        payload = build_search_payload(offset, size, singles_only=singles_only)
        resp = request_with_retries(
            session, "POST", SEARCH_URL, base_delay,
        ...
        if resp is None:
            print(f"Search failed at offset {offset}: no response after retries "
                  f"(network error or hard block). Stopping.", file=sys.stderr)
            return
        if resp.status_code != 200:
            snippet = (resp.text or "")[:600].replace("\n", " ")
            print(f"Search failed at offset {offset}: HTTP {resp.status_code}.\n"
                  f"  Response snippet: {snippet}", file=sys.stderr)
            if resp.status_code == 403:
                print("  -> 403 = Cloudflare/bot block. See the README "
                      "'Troubleshooting 403' section.", file=sys.stderr)
            return
        try:
            data = resp.json()
        except ValueError:
            print(f"Search returned non-JSON at offset {offset} "
                  f"(HTTP {resp.status_code}); likely a challenge page. Stopping.",
                  file=sys.stderr)
            return
        block = (data.get("results") or [{}])[0]
        results = block.get("results", [])
        if total is None:
            total = block.get("totalResults", 0)
            print(f"Total One Piece products reported: {total}")
        if not results:
            return
        for product in results:
...
def _attr(ca, *keys):
    ...
    customAttributes values are sometimes lists (e.g. color=["Purple"],
    cardType=["Character"]); those are flattened to a comma-joined string.
    """
    for k in keys:
...
def upsert_card(conn, product, image_path):
    ...
    conn.execute(
        ...
        INSERT INTO cards (
            ...
            card_number, color, card_type, subtypes, life, power, cost,
            counter, attribute, description, product_url, image_url, image_path,
            market_price, lowest_price, lowest_price_ship, total_listings,
            custom_attributes, raw_json, scraped_at, history_fetched
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,
...
def main():
    ap = argparse.ArgumentParser(description="Scrape One Piece cards from TCGplayer.")
